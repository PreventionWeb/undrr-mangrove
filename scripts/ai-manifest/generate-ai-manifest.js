...
- Arabic: Noto Kufi Arabic for headings (\`$mg-font-family-arabic-headings\`) and Noto Sans Arabic for body (\`$mg-font-family-arabic-body\`). Applied automatically via \`:lang(ar)\` selectors when \`lang="ar"\` is set on a container. Heading/label/UI-chrome components must include a matching \`:lang(ar)\` block since Roboto Condensed has no Arabic glyph coverage. Only Regular and Bold are published in the UNDRR asset library for either family, so Arabic has a 400/700 ladder where Latin has four steps. The ladder is asymmetric: \`font-weight: 600\` resolves up to Bold, \`500\` resolves down to Regular.
