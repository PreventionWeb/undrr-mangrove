...
The public Aria stylesheet currently includes spike-demo composition selectors
such as `.aria-crud-*` and `.aria-integration-demo*`. Before production
adoption, separate those fixtures from the reusable React Aria primitive and
state styles so the distributed surface remains focused.
