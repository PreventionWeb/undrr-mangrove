  ...
  return (
    ...
    >
      <DialogTrigger>
        ...
        <Modal>
          <Dialog>
            {({ close }) => (
              <>
                <Heading slot="title">Add hazardous event</Heading>
                <TextField isRequired value={newHazard} onChange={setNewHazard}>
                ...
                <Select
                  ...
                  onSelectionChange={key => setNewStatus(String(key))}
                >
      ...
      <ResizableTableContainer className="aria-spike-table-scroll">
        ...
        >
          <TableHeader>
            <Column id="selection" width={48} aria-label="Select rows">
              <Checkbox
                slot="selection"
                aria-label="Select all visible events"
                isSelected={pageEvents.every(event =>
                  selectedEvents.has(event.id)
                )}
                onChange={isSelected =>
                  pageEvents.forEach(event =>
                    toggleSelected(event.id, isSelected)
                  )
                }
              />
            </Column>
            <Column id="hazard" isRowHeader allowsSorting defaultWidth={224}>
              {({ sortDirection }) => (
            ...
            <Column id="status" allowsSorting defaultWidth={192}>
              {({ sortDirection }) => (
                <div className="aria-spike-column-header">
                  <span>
                    Status{' '}
            ...
            <Column id="updated" allowsSorting defaultWidth={160}>
              {({ sortDirection }) => (
                <div className="aria-spike-column-header">
                  <span>
                    Updated{' '}
              ...
              >
                <Cell>
                  <Checkbox
                    ...
                    isSelected={selectedEvents.has(item.id)}
                    onChange={isSelected => toggleSelected(item.id, isSelected)}
                  />
                </Cell>
                <Cell>{item.hazard}</Cell>
