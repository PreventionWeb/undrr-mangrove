...
Use `ErrorPage` for common HTTP states with clear, actionable guidance. Adapt links (for example, "Go to home", "Contact support", or "Status page") to your own URLs and support channels.
...
- **Include UNDRR analytics code** to track error events and user behavior:
  `<script type="text/javascript" src="https://assets.undrr.org/analytics/v1.0.0/google_analytics_enhancements.js" crossorigin="anonymous" referrerpolicy="no-referrer"></script>`
...
The Cloudflare Turnstile widget (which replaces `::CAPTCHA_BOX::`) renders at 300 × 65 px (normal) or full-width with min 300 px (flexible mode), is [WCAG 2.1 Level AA compliant](https://developers.cloudflare.com/turnstile/frequently-asked-questions/), and usually passes managed challenges without interaction.

> **Implementation note (for contributors):** The neutral background (`#f2f2f2`), teal accent, and centered card intentionally signal a verification checkpoint (not an error) and reduce CAPTCHA friction. Design references:
>
...
- **Deferred asset loading** to work around Cloudflare asset bundling limits
- **Cloudflare error tokens** for support/debug request details
- **Client-side URL capture** because Cloudflare has no URL token
- **Analytics meta tags** (`vf:page-type`) for page-view tracking
...
```text
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/5xx.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/challenge.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/managed-challenge.html
```

**Individual error pages** (for platforms that support per-code configuration):

```text
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/401.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/403.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/404.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/429.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/500.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/502.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/503.html
https://assets.undrr.org/mangrove/2.0.0-rc.3/error-pages/504.html
```
...
- **1.4.4**, 2026-09-23: Copy no longer uses the first person, following the [Editorial manual](https://mangrove.undrr.org/?path=/docs/contributing-editorial-manual--docs). Applies to the default copy in the component and to all eleven static templates. The 404 heading is now "This page can't be found"; the 500 heading drops "on our side" and its description no longer repeats the contact prompt; the 5xx description starts "Your request couldn't be completed"; and the contact links read "contact UNDRR" instead of "contact us".
- **1.4.3**, 2026-09-17 ([#1190](https://github.com/unisdr/undrr-mangrove/pull/1190)): The UNDRR logo link now carries `aria-label="UNDRR home"`. Its only child is an `aria-hidden` div, so the link had no accessible name at all (axe `link-name`; WCAG 2.4.4 / 4.1.2). Fixed in the component and, separately, in all eleven static templates served by Cloudflare. The two are not generated from one source and their markup differs (the templates inline their own stylesheet and render `mg-error-page__container` where the component renders `mg-error-page__container mg-container--spacer`), so the logo anchor has to be kept in step by hand. Links inside the body copy are also underlined, since colour alone did not distinguish them from the surrounding text (WCAG 1.4.1).
- **1.4.2**, 2026-09-09: Asset URLs in the stylesheet and static error templates now use the canonical CDN root.

- **1.4.1**, 2026-09-01 ([#1086](https://github.com/unisdr/undrr-mangrove/pull/1086)): Replaced the challenge panel's hard-coded radius and shadow with shared card surface tokens so it follows theme expression.
