...
- **Video support**: Use `.mg-embed-container` wrapper for responsive iframes (see [EmbedContainer](?path=/docs/components-embedcontainer--docs))
...
<!-- Combining variants -->
<div
  class="mg-highlight-box mg-highlight-box--secondary mg-highlight-box--centered"
>
...
- **1.2.1** — 2026-09-17 ([#1193](https://github.com/unisdr/undrr-mangrove/pull/1193)): The "With image" story loaded `via.placeholder.com`, a service that no longer resolves, so the story and the docs page showed a broken image. It now uses `picsum.photos`, as every other story in the library does.
...
- **1.1** — 2026-03-10: Removed `.mg-highlight-box:has(iframe)` rule that broke layout on Safari/WebKit mobile. Video embeds now use the [`.mg-embed-container`](?path=/docs/components-embedcontainer--docs) utility wrapper instead of applying aspect ratio to the whole container. [`web-backlog#2672`](https://gitlab.com/undrr/web-backlog/-/work_items/2672)
