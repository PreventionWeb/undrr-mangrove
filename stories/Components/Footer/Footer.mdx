...
For page-level usefulness reporting, stack the standalone [`UserFeedback`](?path=/docs/components-user-feedback--docs) component immediately before `Footer` as an adjacent sibling. Do not nest them. Keeping them separate leaves feedback URLs, analytics and consent product-owned.
