...
- **1.6.0** — 2026-09-10 ([#1114](https://github.com/unisdr/undrr-mangrove/issues/1114)): **Fixed:** every header emitted the same hard-coded element ids, so a page rendering two of them produced duplicate ids and every language `<label>` after the first pointed at the wrong `<select>` (WCAG 1.3.1, 4.1.1). The new `idPrefix` prop namespaces `header`, `block-undrrlogo`, the language form, its select and its no-script submit. Output is byte-for-byte unchanged when `idPrefix` is omitted, so existing Drupal styling and scripting that targets these ids keeps working.

- **1.5.1** — 2026-09-09: The default logo URL now uses the canonical assets CDN root. Custom logo URLs remain supported.

- **1.5** — 2026-09-09 ([#1105](https://github.com/unisdr/undrr-mangrove/issues/1105)): Refined toolbar spacing and divider, added visible focus and 40px controls, and corrected duplicate logo breakpoints without cropping the logo. Language controls follow writing direction; the toolbar retains its 10px inline gutter at every breakpoint, including desktop. Existing decoration and toolbar background are retained.
