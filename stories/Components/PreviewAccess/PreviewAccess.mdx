...
<script
  type="module"
  src="https://assets.undrr.org/mangrove/2.0.0-rc.2/js/preview-access.js"
></script>
```
...
import {
  mgPreviewAccess,
  mgPreviewAccessDestroy,
} from '@undrr/undrr-mangrove/js/preview-access.js';
...
- **1.0.3** — 2026-09-19 ([#1267](https://github.com/unisdr/undrr-mangrove/issues/1267)): The Option B React snippet imported `@undrr/undrr-mangrove/js/preview-access` without the extension, which fails with `ERR_MODULE_NOT_FOUND` in Node ESM — the package ships no `exports` map to fill it in. It now names `js/preview-access.js`. Documentation only; the module is unchanged.
