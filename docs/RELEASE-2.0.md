...
**It is an opt-in entry point.** The normal Mangrove theme stylesheets do not include the React Aria rules. Consumers load `aria/react-aria.css` with one of the per-brand `aria/tokens/{brand}.css` files; both are available from `dist/`, the npm package and the CDN. Token files use zero-specificity `:where(:root)` so Mangrove's own palette wins whatever the load order. The surface is **unlayered**, so it beats a consumer's `@layer` utilities; see [Cascade layers](https://github.com/unisdr/undrr-mangrove/blob/main/docs/CASCADE-LAYERS.md).

**The surface reads a `--mg-aria-*` token contract.** An adapter tier maps Mangrove's palette and component tokens onto the values the React Aria classes read. Brand-neutral constants — type metrics, the shape and size constants shared by the range and toggle family, dialog geometry — live in a Sass mixin emitted at zero specificity inside each standalone token file. `--mg-aria-color-border` now covers only *control* boundaries, where WCAG SC 1.4.11 applies; a new `--mg-aria-color-rule` carries *structure* — table rules, tab rails, dividers, card and overlay edges — at the value the legacy `.mg-tabs__list` rule already uses, so the two surfaces agree by construction. A dangling adapter reference fails the build.
