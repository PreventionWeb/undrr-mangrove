...
Mangrove 2.0 is a major refresh of the library. Theming moved to runtime CSS custom properties, design tokens now come from versioned sources with automated drift checks, core components went through a coordinated interaction and accessibility pass, and the library gained a set of new primitives and page-level patterns.

> **If you consume the base UNDRR compiled CSS (CDN or prebuilt), no sub-brand theming:** The theming migration requires no integration change, and the component refresh keeps existing Drupal hydration and BEM contracts, with a few exceptions in the [upgrade notes](#upgrade-notes): hand-authored heroes with no background image need `mg-hero--no-image`, and hand-written Snackbar markup changes. A page on a 10px root, or loading a `*-legacy.css` stylesheet, must move to a 16px root ([breaking change #5](#5-legacy-10px-root-removed)). Three visual defaults change everywhere regardless (see below).
>
...
>
> **DELTA consumers: your palette changed.** DELTA's Mangrove theme used a navy that DELTA's own design does not use. 2.0 replaces it with the palette from DELTA's production code and design files, the one theme whose compiled output moves beyond the three shared defaults above. See [DELTA's palette is corrected](#deltas-palette-is-corrected).
>
> **If you consume a sub-brand compiled stylesheet** (PreventionWeb, IRP, MCR2030, DELTA): one required change. Add the matching `mg-theme-*` class to `<body>` or a wrapping element, or components fall back to the default UNDRR palette. See [Sub-brand theming migration](#sub-brand-theming-migration).
>
...
2.0 is the `latest` release on npm, so a plain install now resolves to it. Pin the major if you are not ready to upgrade a 1.x project yet (`npm install @undrr/undrr-mangrove@^1`).

**1.x is now in best-effort support.** There is no schedule for further 1.x releases, and new features and fixes land in 2.x. Plan your upgrade with the [upgrade checklist](#upgrade-checklist).

```bash
npm install @undrr/undrr-mangrove@^2
```

The React components need React and ReactDOM 18 or later, installed by your project; 2.0 is developed and tested on 19.3.0. npm does not warn about a missing or older React, so check your version. Compiled CSS, SCSS and the vanilla `js/` modules need no React.

```html
<!-- CDN, versioned path. Load one only: the base stylesheet or a sub-brand one -->
<link rel="stylesheet" href="https://assets.undrr.org/mangrove/2.0.0/css/style.css">
<link rel="stylesheet" href="https://assets.undrr.org/mangrove/2.0.0/css/style-preventionweb.css">
```

For most compiled-CSS consumers the upgrade is two lines: swap the stylesheet href above, and for a sub-brand add `class="mg-theme-{brand}"` (`mg-theme-preventionweb`, `mg-theme-irp`, `mg-theme-mcr` or `mg-theme-delta`, matching the `style-{brand}.css` you load) to `<body>` or a wrapping element. The exceptions are listed in [Find your path](#find-your-path). Found a problem? See [where to report](#feedback).
...
| I am... | Go to... |
|---|---|
| **Anyone upgrading from 1.x** | [Upgrade checklist](#upgrade-checklist): each step says when it applies to you |
| A **product owner or manager** deciding when to upgrade | [Install](#install): pinned 1.x installs and CDN paths keep working, and 1.x is in best-effort support; for hosted CSS the upgrade is one link and, for a sub-brand, one class |
| **New to Mangrove** | [Introduction](https://mangrove.undrr.org/) |
...
| A **sub-brand** Drupal/CDN consumer (PreventionWeb, IRP, MCR2030, DELTA) | [Sub-brand theming migration](#sub-brand-theming-migration): add the `mg-theme-*` body class |
| A **DELTA** consumer | [DELTA's palette is corrected](#deltas-palette-is-corrected): your brand colours changed |
| A site on a **10px root** or loading a **`*-legacy.css`** stylesheet | [Breaking change #5](#5-legacy-10px-root-removed): the legacy stylesheets are gone; move to a 16px root |
| A developer still importing the **`Pagination`** SCSS | [Breaking change #6](#6-deprecated-pagination-component-removed): switch to `Pager` |
...
| A developer **upgrading from a 2.0 alpha, beta or release candidate** | [Upgrade notes](#upgrade-notes): what changed inside 2.0 itself, grouped by integration topic |
...
Work through these in order. Each step says when it applies; skip the ones that don't.

1. **Pin any `latest/` CDN URL.** Applies if a page loads `assets.undrr.org/mangrove/latest/`, which moves to 2.0 with this release. Pin it to `mangrove/1.8.2/` now, and to `mangrove/2.0.0/` once the steps below are done.
2. **Update the version.** Change CDN URLs to `https://assets.undrr.org/mangrove/2.0.0/`, or run `npm install @undrr/undrr-mangrove@^2`.
3. **Load one Mangrove stylesheet per page.** Applies if a page loads both `style.css` and a sub-brand stylesheet. Load one, or `style-all.css` for several brands. See [Multi-brand pages and the combined bundle](#multi-brand-pages-and-the-combined-bundle).
4. **Add the theme class.** Applies if you load `style-preventionweb.css`, `style-irp.css`, `style-mcr.css`, `style-delta.css` or `style-all.css`. Add `mg-theme-preventionweb`, `mg-theme-irp`, `mg-theme-mcr` or `mg-theme-delta` to `<body>` or a wrapping element, or the page renders in the UNDRR palette. See [Sub-brand theming migration](#sub-brand-theming-migration).
5. **Move off a 10px root.** Applies if you load a `*-legacy.css` stylesheet, set `html { font-size: 62.5% }` or `10px`, or set `$mg-html-font-size: 10`. The legacy stylesheets are gone. See [breaking change #5](#5-legacy-10px-root-removed).
6. **Rebuild your Sass and clear every error and warning.** Applies if you import Mangrove SCSS. Removed variables fail the build or print a warning that starts `Mangrove 2.0:`, and each names its replacement. See [Breaking changes](#breaking-changes).
7. **Check button colour overrides.** Applies if your CSS sets `--mg-color-button-background`, `--mg-color-button-background--hover` or `--mg-border-color-button`. They now take a complete colour, not RGB channels; an override written as channels is dropped silently, with no warning. See [breaking change #7](#7-three-button-tokens-changed-shape).
8. **Mark heroes that have no image.** Applies if you hand-author hero markup without a background image. Add `mg-hero--no-image`, or phones show an empty band of brand colour. See [Hero composition and image-less markup](#hero-composition-and-image-less-markup).
9. **Review hand-written markup for changed components.** Applies if you hand-author Snackbar markup, pass links with `target` to QuoteHighlight, or import the removed `Pagination` SCSS. See [Snackbar](#snackbar), [Quote HTML and caller-owned content](#quote-html-and-caller-owned-content) and [breaking change #6](#6-deprecated-pagination-component-removed).
10. **Check React and use subpath imports.** Applies if you import React components. They need React and ReactDOM 18 or later. Import from `@undrr/undrr-mangrove/components/{Name}.js`; the package has no root entry point. See [Published package imports](#published-package-imports).
11. **Check the result.** Look at key pages at phone width and move through them with the keyboard. Expect the changes in [What changed visually](#what-changed-visually), and nothing else.

## What's new in 2.0

If you only need the highlights, this section has them. Each group links to the fuller technical detail further down.
...
- Three system-wide visual defaults changed, in every theme: cool-tinted neutral surfaces, a non-brand gold focus ring and a two-band focus treatment. DELTA's palette was corrected to match DELTA's own production code and design files.
...
- A perceptual (Oklab) colour-contrast methodology runs alongside WCAG 2 on the foreground and background token pairs the contrast tests grade, which now include the focus ring and the data-visualization palette.

See [Theming, tokens and colour](#theming-tokens-and-colour).
...
- Switches gained a pending state for changes that save asynchronously, an error state matching every other form control, and size hooks that derive every length from one number.

See [Component experience and interaction](#component-experience-and-interaction).
...
- Mangrove's interactive components have a focus indicator that stays visible in forced-colours (high-contrast) modes, and an accessibility sweep named, announced and visually distinguished controls across six components.
- One library-wide rule for markerless lists keeps the `list` role Safari otherwise drops, so item counts stay announced without `role="list"` in consumer markup.

See [Typography, language and accessibility](#typography-language-and-accessibility).

**New components and patterns**

- Content Hub, Landing Page and Article Story reference patterns give consumers reference page layouts to compose production pages from.
...
- Snackbar now renders through Notice, and StatusLabel indicators carry a shape as well as a colour.

See [New components and patterns](#new-components-and-patterns).
...
- `js/copy-button.js`, `js/drawer.js`, `js/hub-header.js` and `js/switch-pending.js` are dependency-free ES modules for pages with no React at all, and `js/switch-pending.js` comes apart into its announcer, its revert and its deadline so an app can keep the parts it wants.
- Hydration id prefixes are unique page-wide, and vanilla tab sets removed from the page are suspended rather than left running.

See [Hydration and vanilla scripts](#hydration-and-vanilla-scripts).
...
- A links test fails the build on a documentation link that resolves to no story.

See [Documentation, tooling and discoverability](#documentation-tooling-and-discoverability).
...
Upgrading from 1.x, you will see these changes:

- cool-tinted neutral surfaces: the neutral ramp's surface half takes a tint that is subtle per surface and cumulative across a page ([Theming, tokens and colour](#theming-tokens-and-colour))
- a gold keyboard focus ring, no longer the brand colour, drawn as two bands
- the component refresh, which intentionally changes the presentation and interaction of core components: softer theme-aware surfaces, clearer focus and hover states, more consistent responsive spacing, joined form actions and stronger multilingual behaviour; semantic shapes remain component-specific, so carousel controls stay circular, chips retain meaningful pill geometry and institutional chrome continues to express its owning theme ([Component experience and interaction](#component-experience-and-interaction))
- Snackbar as a light Notice surface rather than a filled coloured bar ([Snackbar](#snackbar))
- filled buttons that paint their fill under the border, so they match outline buttons in size ([Filled buttons and button height](#filled-buttons-and-button-height))
- StatusLabel indicators that carry a shape as well as a colour ([StatusLabel and EmptyState](#statuslabel-and-emptystate))
- background heroes that stack image and copy up to 900px viewport width ([Hero composition and image-less markup](#hero-composition-and-image-less-markup))
- on DELTA sites, DELTA's palette corrected against its own design sources, with buttons going from unfilled navy text to white on brand blue ([DELTA's palette is corrected](#deltas-palette-is-corrected))

Otherwise colours and spacing are visually equivalent to 1.x: the values moved from SCSS variables to CSS custom properties, but the compiled `style.css` output is equivalent. The token rewrite carries the values it started with, apart from [three tokens that changed shape](#7-three-button-tokens-changed-shape).

**Sub-brand consumers (PreventionWeb, IRP, MCR2030, DELTA) have one required change.** Brand colours previously baked into every component rule at compile time; they now live in a `.mg-theme-{brand}` selector block. Add `class="mg-theme-{brand}"` (`mg-theme-preventionweb`, `mg-theme-irp`, `mg-theme-mcr` or `mg-theme-delta`) to `<body>` or a wrapping element, or components render with the default UNDRR palette instead of the brand palette. See [Sub-brand theming migration](#sub-brand-theming-migration).
...
| ------------------------ | --------- | ----------- | -------------- | --------------------------- |
| `--mg-color-neutral-0`   | `#ffffff` | _unchanged_ | `L 1.0000 C 0` | _unchanged_                 |
...
| `--mg-color-neutral-500` | `#666666` | _unchanged_ | `L 0.5103 C 0` | _unchanged_                 |
| `--mg-color-neutral-600` | `#4d4d4d` | _unchanged_ | `L 0.4202 C 0` | _unchanged_                 |
| `--mg-color-neutral-700` | `#333333` | _unchanged_ | `L 0.3211 C 0` | _unchanged_                 |
| `--mg-color-neutral-800` | `#1a1a1a` | _unchanged_ | `L 0.2178 C 0` | _unchanged_                 |
| `--mg-color-neutral-900` | `#000000` | _unchanged_ | `L 0 C 0`      | _unchanged_                 |

Four decisions, each deliberate:

- **Only the surface half moves.** The "generic" reading lives on surfaces. The dark half is body text (`neutral-800`), muted text (`neutral-500`), rules and shadows, where a hue shift buys nothing and every text contrast ratio in the system would have to be re-argued. GOV.UK draws the same line: tinted background, near-black text. Primer tints both; Mangrove did not follow it there.
...
| `--mg-color-gold-800`           | _new_                         | `134 98 0`                  |
| `--mg-color-focus-ring-inverse` | _new_                         | `var(--mg-color-neutral-0)` |
...
**Migration.** Nothing to do unless you depend on the ring being brand-coloured. If you do, set `--mg-color-focus-ring` on `:root` or on your theme selector; a theme that overrides it owns both graded pairs above. `--mg-color-form-focus`, which drives the focused field's border, is unchanged and stays brand-coloured on purpose: the border says "this field is active", the ring says "the keyboard is here".
...
**Focused controls in every theme look slightly different.** The default focus indicator is now two bands (a `neutral-0` separator against the control, then the ring), where it was a single outline on most components and a hand-rolled two-band on a handful.
...
- **A checked radio had no focus indicator at all** (WCAG 2.4.7 Focus Visible, level A). The focus style was a `box-shadow` glow; the radio's centre dot is an inset `box-shadow` declared later at equal specificity, so on a checked radio the dot rule replaced the entire indicator. Nothing errored and nothing was visible. A higher-specificity rule now declares both shadows together.
...
### A data-visualization palette for the Sendai Framework targets

`--mg-sendai-target-a` through `--mg-sendai-target-g` express the seven Sendai Framework targets.

The old `--sendai-red`, `--sendai-orange`, `--sendai-purple` and `--sendai-turquoise` tokens, and the matching `.mg-u-background-color--sendai-*` and `.mg-u-color--sendai-*` utility classes, are **deprecated**: they are brand accent hues named by colour and carry no Sendai Framework meaning. They continue to work unchanged and no build warning flags them, so search your CSS and templates for `--sendai-` and `sendai-` utility classes. Scheduled for removal in 2.1.
...
DELTA's Mangrove theme was built on a navy, `#132e48`, that appears **zero times** in DELTA's own codebase. Its card surface was `#fafafa`, which appears once in DELTA where the colour DELTA actually uses, `#f2f2f2`, appears eleven times. 2.0 reads the palette from DELTA's production repository and the design file for DLDTS, DELTA's design system, instead of inferring it.
...
PreventionWeb, MCR2030 and IRP now define their own secondary button background and hover tokens in `tokens/*.yaml`, instead of inheriting the base values. ([#1150](https://github.com/unisdr/undrr-mangrove/pull/1150))
...
2.0 applies a shared experience direction across the library ([#1086](https://github.com/unisdr/undrr-mangrove/pull/1086)) without mechanically homogenizing components or overriding theme-owned expression:
...
### Why the Dubai typeface was replaced
...
One objection was recorded at the time: dropping Kufi was a judgement rather than a correction, because the UAE Federal Design System and OCHA's own base theme both set Noto Kufi for body.

**That objection was upheld.** Dubai is no longer shipped; the next section describes the replacement.
...
`Notice` is a messaging foundation: `info`, `warning`, `negative` and `positive` severities, compact, prominent and overlay modifiers, and hydration. `ServiceNotice` builds on it for degraded or offline embeds, with capped automatic retry and translated labels in the six official United Nations languages and Japanese. **Snackbar now renders through Notice**, which is a visible change; see the [upgrade notes](#upgrade-notes). ([#1164](https://github.com/unisdr/undrr-mangrove/pull/1164))
...
A copy-to-clipboard button ([#1160](https://github.com/unisdr/undrr-mangrove/pull/1160)) with React hydration, a zero-dependency vanilla script and translated labels in the six official United Nations languages and Japanese ([#1163](https://github.com/unisdr/undrr-mangrove/pull/1163)). It reports a failed copy with a manual-copy hint ([#1166](https://github.com/unisdr/undrr-mangrove/pull/1166)).
...
**Cookie consent tolerates unmount during loading.** _Available from `2.0.0-rc.3`._ A discarded mount cannot initialize the banner after its async asset loads complete, cleanup stays within banner-owned elements, and asset URLs no longer defeat the duplicate-load guard with a cache buster. CookieConsentBanner is a repository integration wrapper, not an npm component bundle. ([#1271](https://github.com/unisdr/undrr-mangrove/pull/1271))
...
**HubHeader gains a vanilla runtime.** _Available from `2.0.0`._ `js/hub-header.js` enhances static hub header markup marked `data-mg-js-hub-header`: it scrolls the current section into view on narrow screens (on load and once web fonts arrive, never on a later resize, so a reader's own scrolling is kept) and drives the edge fades, which CSS cannot do alone. Add `data-mg-hub-header-detect-current` and it also marks the current section from the URL, so one block of markup can be copied to every page of a hub; markup that already marks a section link is left alone. For headers added later, import `mgHubHeader`. The React `HubHeader` runs the same script. See [Content Hub](https://mangrove.undrr.org/?path=/docs/patterns-content-hub--docs), "Without React". ([#1292](https://github.com/unisdr/undrr-mangrove/pull/1292), [#1293](https://github.com/unisdr/undrr-mangrove/pull/1293))

`js/copy-button.js` enhances `data-mg-copy-button` markup on load and exports `mgCopyButton(scope)` for content added later, with no React at all. ([#1163](https://github.com/unisdr/undrr-mangrove/pull/1163))

`js/switch-pending.js` drives the switch pending state. It is on the CDN (`https://assets.undrr.org/mangrove/2.0.0/js/switch-pending.js`) and in npm (`@undrr/undrr-mangrove/js/switch-pending.js`). While a save is pending, it announces "Saving…", "Still saving…" for ignored presses and the outcome in a `role="status"` region, ignores clicks and changes, lets only the latest request settle and reverts on failure or after the timeout (10s by default). Switches marked `data-mg-switch-pending` are enhanced on page load.

- `mgSwitchPending(input, { save, status, timeout, labels, signal })` returns `{ destroy() }`.
- Without a `save` function, a cancelable `mg-switch:save` event lets one site-wide listener answer every switch with `event.detail.respondWith(promise)`. An `async` listener passes an immediately invoked async function (IIFE), or calls `event.preventDefault()` first and `respondWith()` later.
- `mgSwitchPendingInit(scope, options)` and `mgSwitchPendingDestroy(scope)` suit Drupal behaviors.
- The module dispatches `mg-switch:pending`, `mg-switch:settled` and `mg-switch:failed`.
- An explicit call takes over a switch that auto-init enhanced, and duplicate copies of the script share one registry.
...
CDN URLs were canonicalized to a single `assets.undrr.org` root across documentation, examples, font URLs and component defaults. ([#1107](https://github.com/unisdr/undrr-mangrove/pull/1107))
...
**An editorial manual, with its own manifest.** [Contributing/Editorial manual](https://mangrove.undrr.org/?path=/docs/contributing-editorial-manual--docs) sets out the mechanical style rules for UI copy and documentation: capitalization, punctuation, numbers and dates, abbreviations, UNDRR terminology, disability inclusive language and gender-bias review. Each rule cites its source, from the UN Geneva Web Style Guide and the United Nations Editorial Manual to the UNDRR Publications SOP. A separate `llms-editorial-manual.txt` lets an agent writing copy fetch only those rules; `llms.txt`, `llms.json` (`urls.editorialManual`) and the AI integration guide link to it. ([#1286](https://github.com/unisdr/undrr-mangrove/pull/1286), [#1289](https://github.com/unisdr/undrr-mangrove/pull/1289), [#1290](https://github.com/unisdr/undrr-mangrove/pull/1290))
...
`docs/RELEASES.md` now defines a standard format and template for GitHub Release notes (intro, full-detail link and a fixed section order), so the per-release record stays consistent. ([#1186](https://github.com/unisdr/undrr-mangrove/pull/1186))
...
**Browser assertions are now an executable gate.** The build workflow runs Storybook's play functions and render smoke tests in Chromium; a failed assertion fails that step. Run `yarn test-storybook` against a served build locally while organization-level CI is blocked. The accessibility addon uses WCAG A/AA tags but remains advisory: its warnings are not a passing accessibility certification, and cold-page and post-interaction audits cover different states. Lint coverage now includes component JSX, `src/` and `.storybook/`. ([#1272](https://github.com/unisdr/undrr-mangrove/pull/1272), [#1247](https://github.com/unisdr/undrr-mangrove/pull/1247), [#1276](https://github.com/unisdr/undrr-mangrove/pull/1276))
...
| Old | New |
|---|---|
| `$mg-color-interactive` | `rgb(var(--mg-color-interactive))` |
...
`hero.scss` stores variant colours as CSS custom property name strings (`"--mg-color-red-800"`) rather than resolved SCSS values, and the secondary variant reads `--mg-color-hero--secondary`. Consumers that previously overrode `$mg-color-hero--secondary !default` must instead override `--mg-color-hero--secondary` on `:root` or in a `.mg-theme-*` block. The secondary hero now shows that colour as a rule along its leading edge rather than as a fill (see [The orange accent carries no text](#the-orange-accent-carries-no-text)).
...
:root {
  --mg-color-hero--secondary: 179 71 0; // channels for #b34700
}
```
...
The deprecated `Pagination` component, its stories and documentation are removed. In 1.8.x it had no compiled CSS or published component bundle, so only a build that imports `scss/Components/Pagination/pagination` or copies its source is affected. Use `Pager`, which is the supported 2.0 pagination pattern.
...
**Two of these break overrides.** Consumers commonly set `--mg-color-button-background` and `--mg-color-button-background--hover` in their own themes. An override still written as channels is now invalid and the browser drops it silently, with no build warning: buttons lose their fill. Wrap the channels in `rgb()`:
...
**`--mg-border-color-button` is a fix.** It held channels but was consumed as `border: var(--mg-border-width-button) solid var(--mg-border-color-button)`. A triple there makes the whole `border` shorthand invalid, so `border-style` fell back to `none` and `.mg-button-outline` rendered as bare coloured text in every theme. The build's legacy-override warning covers only the `$mg-border-color-button` Sass variable; a CSS override of `--mg-border-color-button` written as a triple gets no warning.
...
The [breaking changes](#breaking-changes) above are the 1.x-to-2.0 migration. These are the smaller adjustments 2.0 asks for, whether you are arriving from 1.x or from a 2.0 alpha, beta or release candidate. They are grouped by what you touch, not by the prerelease they landed in.
...
_Available from `2.0.0-rc.3`._ QuoteHighlight sanitizes `quote`, `attribution` and `attributionTitle` with DOMPurify. Normal emphasis and attribution links survive; scripts, event handlers, unsafe URLs and embeds do not. The default policy also strips `target` and `referrerpolicy`, so an attribution link with `target="_blank"` now opens in the same tab. Check authored quotes before upgrading. Tab panel HTML and MegaMenu's `bannerDescription` still require caller-sanitized content, and custom syndicated-search endpoints remain a trust boundary. ([#1261](https://github.com/unisdr/undrr-mangrove/pull/1261), [#1258](https://github.com/unisdr/undrr-mangrove/pull/1258), [#1254](https://github.com/unisdr/undrr-mangrove/pull/1254))
...
Snackbar keeps its props, but the toast renders as a Notice with a light surface instead of a filled coloured bar. If you hand-wrote Snackbar markup, the `.mg-snackbar__content` and `.mg-snackbar__icon` elements and the filled `.mg-snackbar__error`, `__warning`, `__info` and `__success` colour styles are gone (the modifier classes remain only as hooks), and `SnackbarIcons.jsx` is removed. Snackbar was not used in production before this change. `severity` now defaults to `info`, where it was previously unset. `info` and `success` toasts announce politely (`role="status"`), a toast with `openedMiliseconds` no longer takes focus, and focus returns to its previous element on close.

### Type scale

Use `var(--mg-font-size-*)` in new styles. `$mg-font-size-*`, `$mg-font-body` and `$mg-font-tag` still compile but are deprecated until 3.0; overriding `$mg-font-tag` no longer changes Mangrove's own components.
...
_Available from `2.0.0-rc.3`._ React ShowMore owns only its toggle listeners, cleans them up on unmount and initializes items added after the first render. Use `labelCollapsed` (default “Show more”) and `labelOpen` (default “Show less”) for translated labels; `button_text` is legacy. The vanilla helper resolves a target from the nearest ancestor outward before falling back to the document, so repeated default-target instances no longer toggle the first block on the page. ([#1279](https://github.com/unisdr/undrr-mangrove/pull/1279), [#1238](https://github.com/unisdr/undrr-mangrove/pull/1238))

### Prototypes

Drawer, Tree, Legend and Range are prototypes, and their APIs may still change in a 2.x minor release. Drawer's `children` and `footer` strings render as text; pass markup through `bodyHtml` / `footerHtml`, which are sanitized.
...
.mg-theme-mytheme {
  ...
  --mg-color-interactive:        10 105 105;
...
The class can be applied to any ancestor element, not just `<body>`. Multiple theme classes can coexist on different subtrees.
