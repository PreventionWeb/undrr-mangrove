...
Alpha.3 is a foundation release. It adds no required integration change and does not alter how any existing component renders.
...
- `aria/react-aria.css` and `aria/tokens/{brand}.css` are also emitted standalone, for consumers who want the React Aria styling without Mangrove's full stylesheet. The token files are wrapped in `:where(:root)`, which has zero specificity, so Mangrove's own palette still wins when both are loaded regardless of load order.
- `aria/react-aria.layered.css` is the same surface wrapped in a `@layer mangrove` block for consumers who manage cascade layers. See [Cascade layers](https://github.com/unisdr/undrr-mangrove/blob/main/docs/CASCADE-LAYERS.md) — note that unlayered styles beat layered ones, which is the opposite of most expectations.
...
**Alpha.3** changes nothing visually. Its token rewrite is value-identical — the compiled output for all five themes is byte-unchanged — and the new `StatusLabel`, `EmptyState` and React Aria styles are additions that no existing component uses. The only consumer-visible change is a Sass `@warn` for SCSS consumers still using the deprecated `--sendai-*` accent tokens.
