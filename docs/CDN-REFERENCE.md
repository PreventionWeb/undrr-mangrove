...
```html
<script type="module">
  import { mgTabs } from 'https://assets.undrr.org/static/mangrove/2.0.0-alpha.3/js/tabs.js';
  mgTabs();
</script>
```
...
<script type="module">
  ...
  const MegaMenuModule = await import(
    'https://assets.undrr.org/static/mangrove/2.0.0-alpha.3/components/MegaMenu.js'
  );
...
```
https://assets.undrr.org/static/mangrove/2.0.0-alpha.3/css/style.css
```
