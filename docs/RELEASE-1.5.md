...
- **TableOfContents and OnThisPageNav: published as standalone vanilla JS assets:** `table-of-contents.js` and `on-this-page-nav.js` are now published to the CDN, following the same convention as `tabs.js` and `show-more.js`. (Correction: this said `dist/js/` when it was written. Nothing has ever been emitted there — the modules ship as source at `dist/assets/js/`, which is what the CDN and the npm `js/` directory serve. See unisdr/undrr-mangrove#1253.) See the updated [CDN reference](https://mangrove.undrr.org/?path=/docs/getting-started-integration-cdn-reference--docs) for the new module paths. ([#901](https://github.com/unisdr/undrr-mangrove/pull/901))
