...
- Build output is no longer committed. The generated token partials, the five
  `aria/tokens/*.css` files and `aria/react-aria.css` are produced by `yarn scss`
  and are now gitignored, as the compiled theme stylesheets already were. A Jest
  `globalSetup` writes any missing token partial, so `yarn test` works on a fresh
