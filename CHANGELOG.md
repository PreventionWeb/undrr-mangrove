...
- **Storybook "Show code" works on every docs page:** stories written as `render` functions opened an empty code panel because Babel compiled stories to CommonJS, which stopped Storybook reading story source. The CommonJS transform now applies only when Babel runs under Jest (`babel-jest`), whatever `NODE_ENV` or `BABEL_ENV` is set to. Story files for CSS-only components (such as EmptyState, StatusLabel and Tag) opt in with `parameters.docs.source.html: true` and show the story rendered as formatted HTML to copy; other stories keep showing JSX. The HTML renderer (`react-dom/server` and Prettier) is a separate chunk that loads only for opted-in files. ([#1177](https://github.com/unisdr/undrr-mangrove/issues/1177))
...
- **Hydration `identifierPrefix` is unique page-wide:** `createHydrator` numbered each root by its position in the scanned context, so a later `update(context)`, as a Drupal behavior calls it, restarted at 0 and roots could share a prefix. With one shared copy of React, React's own counter still kept `useId` values apart, but a page that loads two copies (for example a site bundle alongside the import-map build) got duplicate DOM ids, such as repeated Tree item ids. Roots now take a page-wide number shared by every hydrator and every copy of the runtime from this release on; a page that mixes this runtime with an older copy can still collide. The option and return value are unchanged; only the number in the generated prefix differs. ([#1178](https://github.com/unisdr/undrr-mangrove/pull/1178))
